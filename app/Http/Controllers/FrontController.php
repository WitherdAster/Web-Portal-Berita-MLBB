...
class FrontController extends Controller
{
    ...
    public function index(){
        ...
        $mdl_entertainment = ArticleNews::whereHas('category', function ($query) {
            $query->where('name', 'MPL');
        })
        ->where('is_featured','not_featured')
        ->latest()
        ->take(6)
        ...
        $mpl_entertainment = ArticleNews::whereHas('category', function ($query) {
            $query->where('name', 'MPL');
        })
        ->where('is_featured','featured')
        ->inRandomOrder()
        ...
        return view('front.index', compact('mpl_entertainment','mdl_entertainment','categories', 'articles', 'authors', 'featured_articles', 'bannerads'));
    }
