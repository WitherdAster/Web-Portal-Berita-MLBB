...
@section('content')
        ...
        <!-- tikas sini malam tadi -->
            ...
            <div class="relative inline-block text-left">
                <button onclick="toggleDropdown()" 
                        class="rounded-full p-[12px_22px] flex gap-[10px] font-bold transition-all duration-300 bg-[#0379F4] text-white hover:shadow-[0_10px_20px_0_#0379F4]">
                    <span>Category ▼</span>
                </button>

                <div id="dropdownMenu" 
                    class="z-50 absolute mt-2 w-50 bg-white border border-gray-200 rounded-md shadow-lg hidden">
                    @foreach ($categories as $category)
                        <a href="{{ route('front.category', $category->slug) }}" class="p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 border border-[#EEF0F7] hover:ring-2 hover:ring-[#0379F4]">
                    ...
                    <div class="featured-news-card relative w-full h-[550px] flex shrink-0 overflow-hidden">
                        ...
                        <div class="card-detail max-w-[1130px] w-full mx-auto flex items-end justify-between pb-10 relative z-20">
                            ...
                            <div class="prevNextButtons flex items-center gap-4 mb-[60px]">
                                <button class="button--previous appearance-none w-[38px] h-[38px] flex items-center justify-center rounded-full shrink-0 ring-1 ring-white hover:ring-2 hover:ring-[#0379F4] transition-all duration-300">
                                    <img src="assets/images/icons/arrow.svg" alt="arrow" />
                                </button>
                                <button class="button--next appearance-none w-[38px] h-[38px] flex items-center justify-center rounded-full shrink-0 ring-1 ring-white hover:ring-2 hover:ring-[#0379F4] transition-all duration-300 rotate-180">
                                    <img src="assets/images/icons/arrow.svg" alt="arrow" />
                                </button>
                            </div>
                        </div>
	...
	<section id="kontainer_author_berita" class="grid-cols-1 max-w-[1130px] mx-auto flex-col gap-[30px] grid sm:grid-cols-[70%_30%] divide-x divide-gray-300">

        <section id="Up-to-date" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px] pl-2">
            <div class="flex justify-between items-center">
                <h2 class="font-bold text-[26px] leading-[39px]">
                    Latest Hot News <br />
                    Good for Curiousity
                </h2>
                <p class="badge-orange rounded-full p-[8px_18px] bg-[#7ddcf4] font-bold text-sm leading-[21px] text-[#0379F4] w-fit">UP TO DATE</p>
            </div>
            <div class="grid grid-cols-1 gap-[30px]">

                    @forelse ($articles as $article)
                        <a href="{{ route('front.details', $article->slug) }}" class="card-news">
                            <div class="rounded-[20px] ring-1 ring-[#EEF0F7] p-[26px_20px] flex flex-col gap-4 hover:ring-2 hover:ring-[#0379F4] transition-all duration-300 bg-white">
		...
		<section id="Best-authors" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px] ml-2 pl-2">
	        <div class="flex flex-col text-center gap-[14px] items-center ">
                <p class="badge-orange rounded-full p-[8px_18px] bg-[#7ddcf4] font-bold text-sm leading-[21px] text-[#0379F4] w-fit">BEST AUTHORS</p>
                <h2 class="font-bold text-[26px] leading-[39px]">
                    Explore All Masterpieces <br />
                    Written by People
                </h2>
	        </div>
	        <div class="grid grid-cols-2 mx-8 sm:grid-cols-1 gap-[30px]">

                @forelse ($authors as $author)
                    <a href="{{ route('front.author', $author->slug) }}" class="card-authors">
                        <!-- container membuat author card -->
                        <div class=" grid-cols-1 rounded-[20px] border border-[#EEF0F7] p-[26px_20px] flex-col grid sm:grid-cols-2 items-center gap-4 hover:ring-2 hover:ring-[#0379F4] transition-all duration-300">
                            <!-- memunculkan gambar dan membuat gambar nya memenuhi container -->
                            ...
                            <!-- detail untuk nama dan jumlah news nya -->
                            <div class="flex flex-col gap-1 text-center">
                                <p class="font-semibold">
                                    {{ $author->name }}
                                </p>
                                <p class="text-sm leading-[21px] text-[#A3A6AE] sm:block">
		...
		<section id="Advertisement" class="max-w-[1130px] mx-auto flex justify-center mt-[70px]">
	        <div class="px-6 lg:px-6 md:px-6 sm:px-0 flex flex-col gap-3 shrink-0 w-fit">
		        <a href="{{ $bannerads->link }}">
                    <div class="sm:w-[900px] h-[120px] flex shrink-0 border border-[#EEF0F7] rounded-2xl overflow-hidden">
                        <img src="{{ route('private.file', $bannerads->thumbnail) }}" class="overflow-hidden md:object-cover w-full h-full" alt="ads" />
                    </div>
		        </a>
		...
		<section id="MPL-entertainment" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			<div class="flex items-center justify-between">
				<h2 class="font-bold text-[26px] leading-[39px]">
					ChampionShip <br />
					in MPL Indonesia
				</h2>
				<a href="categoryPage.html" class="rounded-full p-[12px_22px] flex gap-[10px] font-semibold transition-all duration-300 border border-[#EEF0F7] hover:ring-2 hover:ring-[#0379F4]">Explore All</a>
			</div>
			<div class="flex items-center justify-between h-fit">
				<div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
					<img src="{{ route('private.file', $mpl_entertainment->thumbnail) }}" class="absolute object-cover w-full h-full thumbnail" alt="icon" />
					<div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
					<div class="card-detail w-full flex items-end p-[30px] relative z-20">
						<div class="flex flex-col gap-[10px]">
							...
							<a href="details.html" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">
									{{ $mpl_entertainment->name}}
								</a>
							<p class="text-white">
									{{ $mpl_entertainment->created_at->format('M d,Y') }}
								</p>
						</div>
					</div>
				</div>
				<div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
					<div class="w-[455px] flex flex-col gap-5 shrink-0">
							...
							@forelse ($mdl_entertainment as $article)
								<a href="{{ route('front.details', $article->slug) }}" class="card py-[2px]">
									<div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#0379F4] transition-all duration-300">
